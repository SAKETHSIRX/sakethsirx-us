...
interface GlowCardProps {
  ...
  style?: React.CSSProperties; // Allow inline styles (e.g., animationDelay)
}
...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    ...
    const handleMouseLeave = () => {
      setIsHovered(false);
      // Reset position to center when not hovering
      const rect = card.getBoundingClientRect();
  ...
  const getInlineStyles = () => {
    const baseStyles: React.CSSProperties & Record<string, string | number> = {
      ...
      '--bg-spot-opacity': isHovered ? '0.15' : '0.05',
      '--border-spot-opacity': isHovered ? '1' : '0.3',
      '--border-light-opacity': isHovered ? '1' : '0.1',
