...
const GlowingEffect = memo(
  ({
    ...
    const handleMove = useCallback(
      (e?: MouseEvent | { x: number; y: number }) => {
        ...
        animationFrameRef.current = requestAnimationFrame(() => {
          ...
          // Only animate if element is still valid and not disabled
          if (element && !disabled) {
            try {
              currentAnimationRef.current = animate(currentAngle, newAngle, {
                ...
                ease: [0.16, 1, 0.3, 1],
                onUpdate: (value) => {
                  // Double-check element is still valid during animation
    ...
    useEffect(() => {
      if (disabled) {
        // Clean up any running animation when disabled
        if (currentAnimationRef.current) {
          currentAnimationRef.current.stop();
          currentAnimationRef.current = null;
        }
