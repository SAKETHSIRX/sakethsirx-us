...
export const queries = {
  // User queries
  // Include verification fields for OTP flows
  findUserByEmail: formatQuery(
    'SELECT id, name, email, password, is_active, email_verified, verification_code, verification_expires FROM users WHERE email = ?'
  ),
  ...
  // Generic update for verification data (code + expiry)
  setVerificationData: formatQuery(
    'UPDATE users SET verification_code = ?, verification_expires = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
  ),

  // Clear verification data (after success)
  clearVerificationData: formatQuery(
    'UPDATE users SET verification_code = NULL, verification_expires = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
  ),

  // Mark email as verified
  markEmailVerified: formatQuery(
    'UPDATE users SET email_verified = 1, verification_code = NULL, verification_expires = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
  ),
